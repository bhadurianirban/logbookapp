...
import { GetCommonMasterDataAction, GetReasonRemarksOptionsAction, GetAutoRecloseHistroyAction } from 'src/app/store/actions';
...
@Component({
  selector: 'app-historic-auto-reclose',
...
export class AutoRecloseComponent implements OnInit, OnDestroy {
  ...
  destroying = false;
  historyAutoRecloseData: AutoRecloseModel[];
  ...
  constructor(private store: Store<ApplicationState>,
              ...
              private messageService: MessageService) {
                ...
                this.store.dispatch(new GetAutoRecloseHistroyAction({
  ...
  ngOnInit() {
    ...
    takeWhile(() => !this.destroying))
    .subscribe(data => {
      if (data && data.AutoRecloseData) {
        this.historyAutoRecloseData = data.AutoRecloseData;
      }
    });
  }
  ...
  getReport() {
    if (this.rangeDates && this.rangeDates.length > 1) {
      ...
      this.messageService.add(
        { key: 'errorNotification', severity: 'error', summary: 'Auto Reclose Elements Report',
